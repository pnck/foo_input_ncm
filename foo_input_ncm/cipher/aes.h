...
namespace fb2k_ncm::cipher {
    ...
    public:
        ...
        void load_key(const std::vector<uint8_t> key) {
            ...
            if (!NT_SUCCESS(status = BCryptGenerateSymmetricKey(h_crypt_, &h_key_, NULL, 0, (PUCHAR)key.data(), KEYLEN, 0))) {
    ...
    class AES_context {
        template<aes_chain_mode> struct enum_mapping_impl;
        template<> struct enum_mapping_impl <aes_chain_mode::ECB> { constexpr inline static wchar_t value[] = BCRYPT_CHAIN_MODE_ECB; };
        template<> struct enum_mapping_impl <aes_chain_mode::CBC> { constexpr inline static wchar_t value[] = BCRYPT_CHAIN_MODE_CBC; };
        template<aes_chain_mode M> static inline auto mapped_str = make_mapping_with_impl<M, enum_mapping_impl>::value;
    public:
        template <size_t KEYLEN>
        explicit AES_context(AES<KEYLEN> && aes_crypt/* to consturct the context, the crypt object must not be standalone */) : crypt_(std::move(aes_crypt)) {
    ...
    private:
        ...
        size_t do_decrypt(uint8_t * dst, size_t cb_dst, const uint8_t * src, const size_t cb_src) {
            ...
            if (!NT_SUCCESS(status_ = BCryptSetProperty(h_key, BCRYPT_CHAINING_MODE, (PUCHAR)mapped_str<M>, sizeof(mapped_str<M>), 0))) {
                throw cipher_error("BCryptSetProperty set chain mode failed", status_);
            }
            ULONG required_size = 0;
            if (!NT_SUCCESS(status_ = BCryptDecrypt(h_key, (PUCHAR)src, cb_src, NULL, NULL, 0, NULL, 0, &required_size, 0/*BCRYPT_BLOCK_PADDING*/))) {
                throw cipher_error("Cannot get required size", status_);
            }
            if (!NT_SUCCESS(status_ = BCryptDecrypt(h_key, (PUCHAR)src, cb_src, NULL, NULL, 0, (PUCHAR)dst, cb_dst, &required_size, 0/*BCRYPT_BLOCK_PADDING*/))) {
                throw cipher_error("BCryptDecrypt failed", status_);
            }
            return required_size;
        }
        template <size_t KEYLEN>
        auto internal_get_key_handle(AES<KEYLEN> & _) { return _.h_key_; }
        template <size_t KEYLEN>
        auto internal_get_crypt_handle(AES<KEYLEN> & _) { return _.h_crypt_; }
        template <size_t KEYLEN>
        auto internal_get_key_length(const AES<KEYLEN> &) const { return KEYLEN; }
    ...
    public:
        ...
        inline size_t key_len() const { return std::visit([this](const auto & _) { return internal_get_key_length(_); }, crypt_); }
        ...
        inline size_t inputted_len() const {
            ...
            if (std::holds_alternative<const uint8_t*>(input_buffer_)) { return input_head_ - std::get<const uint8_t*>(input_buffer_); }
            else { return input_head_ - std::get<const std::vector<uint8_t>*>(input_buffer_)->data(); }
        }
        ...
        inline size_t outputted_len() const {
            ...
            if (std::holds_alternative<uint8_t*>(output_buffer_)) { return output_head_ - std::get<uint8_t*>(output_buffer_); }
            else { return output_head_ - std::get<std::vector<uint8_t>*>(output_buffer_)->data(); }
        }
    ...
    template <size_t KEYLEN>
    [[nodiscard]]
    inline AES_context make_AES_context_with_key(const uint8_t(&key)[KEYLEN]) {
        return AES_context(AES(key));
    }
    [[nodiscard]]
    inline AES_context make_AES_context_with_key(const uint8_t* key, size_t key_len) {
        switch (key_len) {
        case 16: {
            AES128 _t;
    ...
    [[nodiscard]]
    inline AES_context make_AES_context_with_key(const std::vector<uint8_t> & key) {
        return make_AES_context_with_key(key.data(), key.size());
    }
    [[nodiscard]]
    inline size_t guess_padding(uint8_t* end) {
        uint8_t last = *(end - 1);
