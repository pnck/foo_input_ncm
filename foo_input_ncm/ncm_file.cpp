...
t_size fb2k_ncm::ncm_file::read(void * p_buffer, t_size p_bytes, abort_callback & p_abort)
{
    if (rc4_decrypter.key_seed_.empty()) {
        ...
        auto read_offset = source_->get_position(p_abort) - audio_content_off_;
...
{
    return source_->get_size(p_abort) - audio_content_off_;
}

t_filesize fb2k_ncm::ncm_file::get_position(abort_callback & p_abort)
{
    return source_->get_position(p_abort) - audio_content_off_;
}
...
{
    return source_->seek(audio_content_off_ + p_position, p_abort);
}
...
void fb2k_ncm::ncm_file::reopen(abort_callback & p_abort)
{
    ...
    source_->seek(audio_content_off_, p_abort);
}
